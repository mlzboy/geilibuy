...
function addAttach() {
	...
	//ceshi
	tags = newnode.getElementsByTagName('div');
	for(i in tags) {
		if(tags[i].id == 'showpicthumb') {
			tags[i].id = 'showpicthumb' + id;
		}
	}
	//ceshi
	aid++;
...
function insertAttach(id) {
	...
	//QUWAN 不允许一张图片多次上传
	var picfilename =  localfile;
	localfile = '<span>' + localfile + '</span>';
	var nownode = $('attachbody');
    pictags = nownode.getElementsByTagName('span');
	for(i in pictags) {
		if(pictags[i].innerHTML == picfilename) {
		    alert('对不起，一张图片仅可以上传一次');
			return;
		}
	}
	//END QUWAN 不允许一张图片多次上传
	...
	if(is_ie || userAgent.indexOf('firefox') >= 1) {
		...
		//inhtml += '<td><img src="' + picPath +'" width="60" height="80">&nbsp;</td>';//去掉 QUWAN IE7 IE8预览本地图片
	}
    ...
    if (is_ie)
    {
        $('showpicthumb' + id).style.cssText="filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod=scale);";
	    $('showpicthumb' + id).filters.item("DXImageTransform.Microsoft.AlphaImageLoader").src = picPath;
	    $('showpicthumb' + id).style.width = "60px";
	    $('showpicthumb' + id).style.height = "80px";
	} else if(is_moz)
	{
	    $('showpicthumb' + id).innerHTML = '<img src="' + picPath +'" width="60" height="80">&nbsp';
	}
    //end QUWAN IE7 IE8预览本地图片
...
function upload() {
	...
	if($('showmsg'+nid) != null) {
		...
		if(typeof (no_insert) != 'undefined') {
			...
			$('uploadpicnum').value = nowUid;//QUWAN 成功上传了几张图片
