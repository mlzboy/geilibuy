 ...
 *
 * @version $Id: jquery.optionTree.js 9 2010-10-29 13:30:17Z kkotowicz $
...
$.fn.optionTree = function(tree, options) {
	...
	set_value_on_last_no_empty_value:false,//if true,if the last select option is choose level x,then it will try to find anastor select's select value give the result text
    }, options || {});
    ...
    var setValue = function(name, value) {
        $("input[id='" + cleanName(name) + "']").val(value).change();
    }
        ...
        if (typeof tree == "object") { // many options exists for current nesting level
            ...
            .change(function() {
                if (this.options[this.selectedIndex].value != '') {
                  ...
                  removeNested(name + '_');
		    if (options.set_value_on_last_no_empty_value)
		    {
                        if ($("select[id='"+name.substring(0,name.length-1)+"']").length>0)
		        {
                             setValue(name, $("select[id='"+name.substring(0,name.length-1)+"']").val());
                        }
		        else
                        {
